def it_was_ok
  ...
  Movie
    ...
    .where(score: 2.0..3.0)
  # Movie.select(:id, :title, :score).where(yr: 1970..1979, score: 2.0..3.0) (WHERE AND)
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group(:id).order('COUNT(castings.movie_id) DESC').limit(3) 
end
